...
URL <- str_glue("https://data.broadinstitute.org/gsea-msigdb/msigdb/release/{date}.Hs/msigdb_v{date}.Hs.db.zip")

# mk db (human)

local_zip_path <- str_glue("./msigdb_v{date}.Hs.db.zip")
download.file(URL, local_zip_path)
print("download success!")
unzip(local_zip_path, exdir = "./")

con <- DBI::dbConnect(RSQLite::SQLite(), dbname = str_glue("./msigdb_v{date}.Hs.db"))
